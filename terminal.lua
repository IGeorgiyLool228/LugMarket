...
local INFO = [[
|0x1e3de8|---=== Пользовательское соглашение ===---|0xffffff|
...
|0xdbd904|Приобритённые товары в магазине обратно не принимаются.]]
...
local function discord()
    setColorText(6, 18, "[0x303030]По любым проблемам пишите в Discord: [0x337d11]Dadya_kvantum", color.background)
end
...
local function balance(y, account)
    ...
    setColorText(nil, y, "[0x68f029]Баланс: [0xffffff]" .. math.floor(session.balance) .. " $", color.background)
end
...
local function lottery()
    ...
    setColorText(19, 4, "[0x68f029]Супер-приз — [0xffffff]" .. superPrize .. " [0x68f029]$!", color.background)
    field()
end
...
local function playLottery()
    if session.balance >= priceLottery then
        ...
        rip = math.floor(rip)
        setColorText(nil, 8, "[0x68f029]Вы выиграли: [0xffffff]" .. rip .. " [0x68f029]$", color.background)
...
local function info()
    set(20, 1, "Соглашение", color.background, color.orange)
    drawInfo(1)
end
...
function login(name)
    if name then
        ...
        if active then
            ...
            setColorText(17, 5, "       [0xffffff]Встаньте на [0x14e162]PIM[0xffffff]!", color.background)
            discord()
            drawPim()
        end
    end
...
buttons = {
    ...
    other = {buttonIn = {"main"}, background = color.gray, activeBackground = color.blackGray, foreground = color.lime, activeForeground = color.blackLime, text = "Прочее ◕⩊◕", x = 19, y = 9, width = 24, height = 3, action = function() toGui("other") end},
    ore = {buttonIn = {"other"}, background = color.gray, activeBackground = color.blackGray, foreground = color.lime, activeForeground = color.blackLime, text = "Обмен руды", x = 19, y = 4, width = 24, height = 3, action = function() toGui("ore") end},
    ...
    buy = {buttonIn = {"shop"}, background = color.gray, activeBackground = color.blackGray, foreground = color.lime, activeForeground = color.blackLime, text = "Купить предметы", x = 19, y = 6, width = 24, height = 3, action = function() toGui("buy") end},
    sell = {buttonIn = {"shop"}, background = color.gray, activeBackground = color.blackGray, foreground = color.lime, activeForeground = color.blackLime, text = "Продать предметы", x = 19, y = 10, width = 24, height = 3, action = function() toGui("sell") end},
    ...
    playLottery = {buttonIn = {"lottery"}, background = color.gray, activeBackground = color.blackGray, foreground = color.lime, activeForeground = color.blackLime, text = "Играть ", x = 19, y = 13, width = 24, height = 3, action = function() playLottery() end},
        ...
        elseif itemScan then
            if guiPath[#guiPath] == "sellItem" then
                ...
                if count and pushItem(slot, count) then
                    ...
                    if response and response.code == 200 then
                        setColorText(nil, 14, "[0x68f029]Баланс успешно пополнен на [0xffffff]" .. math.floor(addMoney) .. " [0x68f029] $", color.background)
                        balance(1)
                        scanMe()
                ...
                for item = 1, #items.ore do
                    ...
                    if count then
                        ...
                        if ingots < count then
                            ...
                            alert({"Слитки закончились, зайдите позже •⩊•"}, drawOreList)
                        else
